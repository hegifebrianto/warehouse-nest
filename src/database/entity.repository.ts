...
type ProjectionType<T> = mongoose.ProjectionType<T>;
...
export abstract class EntityRepository<T extends Document> {
	...
	async findOneAndUpdate(
		...
		const result = await this.entityModel.findOneAndUpdate(entityFilterQuery, updateEntityData, { new: true }).exec();
	...
	async deleteOneById(id: mongoose.Types.ObjectId | string): Promise<T | null> {
		...
		const objectId = new mongoose.Types.ObjectId(id);
		return await this.entityModel.findOneAndDelete({ _id: objectId }).exec();
	}
	...
	async paginate(
		...
		console.log('📄 Pagination Params:', { page, pageSize, skip, orderBy, orderDirection });
